...
const generateABrandNewDeck = (): Deck => {
    return Object.keys(Suit).map(suit => Object.keys(CardValue).map(value => Card(Suit[suit as keyof typeof Suit], CardValue[value as keyof typeof CardValue]))).flat()
}
